<template>
	<div class="dialog-overlay" @click="$emit('close')">
		<div class="dialog-content" @click.stop>
			...
			<div v-if="error" class="error-message">获取版本列表失败: {{ error }}</div>
...
const fetchVersions = async () => {
	...
	try {
		switch (props.loader) {
			case 'forge':
				// Forge API
				const forgeRes = await http.fetch(
			...
			case 'fabric':
				// Fabric API
				const fabricRes = await http.fetch(
					'https://meta.fabricmc.net/v2/versions/loader/' + props.minecraftVersion
				);
				if (fabricRes.ok) {
			...
			case 'neoforge':
				// NeoForge API
				const neoforgeRes = await http.fetch(
					'https://maven.neoforged.net/api/maven/versions/net/neoforged/neoforge'
				);
				if (neoforgeRes.ok) {
					versions.value = neoforgeRes.data.versions.filter((v) =>
						v.startsWith(props.minecraftVersion)
					);
				}
				break;
