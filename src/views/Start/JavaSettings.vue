  ...
  <div class="java-settings-container">
    ...
    <div class="settings-box">
      <div class="form-group">
        ...
        <p class="path-display" v-if="selectedJava">
          路径: {{ selectedJava.path }}
        </p>
    ...
    <div class="actions">
      <button class="back-button" @click="router.push('/start/login')">
        返回
      </button>
      <button 
        class="next-button"
...
const refreshJavaList = async () => {
  try {
    ...
    const newList = [...javaList.value, ...installations.filter(java => 
      !javaList.value.some(j => j.path === java.path)
    )]
    ...
    if (!selectedJava.value && newList.length > 0) {
      const java8_64 = newList.find(java => 
        java.version.startsWith('1.8.') && java.is_64bit
      )
