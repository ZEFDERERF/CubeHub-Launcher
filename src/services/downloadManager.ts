...
class DownloadManager {
  ...
  async downloadForge(minecraftVersion: string): Promise<void> {
    try {
      ...
      // 开始下载
      await this.startDownload(downloadUrl, fileName, `Forge ${minecraftVersion}-${forgeVersion}`)
    } catch (error) {
  ...
  private async getVersionInfo(version: string): Promise<VersionInfo | null> {
    try {
      const response = await fetch(`https://launchermeta.mojang.com/v1/packages/${version}.json`)
      if (!response.ok) {
