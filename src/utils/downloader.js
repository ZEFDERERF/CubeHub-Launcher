...
export class Downloader {
	...
	getVersionCategory(id, type) {
		...
		if (id.startsWith('a') || id.startsWith('b') || id.startsWith('c') || /^0\./i.test(id)) {
			return 'old';
		}

		// 处理快照版本
		if (type === 'snapshot' || /(-pre|-rc|[a-z]+\d*$)/i.test(id)) {
			return 'snapshot';
		}
	...
	async downloadFile(download, versionId) {
		...
		while (true) {
			...
			this.onProgress?.(versionId, {
				...
				percentage:
					(this.progress[versionId].current / this.progress[versionId].total) * 100,
			});
		}
	}
	...
	async getFabricVersions(minecraftVersion) {
		try {
			const response = await fetch(
				`${this.fabricUrl}/v2/versions/loader/${minecraftVersion}`
			);
